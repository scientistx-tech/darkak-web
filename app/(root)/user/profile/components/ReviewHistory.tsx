  ...
  return (
    <div className="mx-auto w-full max-w-5xl rounded-3xl border bg-gradient-to-tr from-[#ffffff80] via-[#ecf3ff90] to-[#ffffff80] p-2 py-4 shadow-2xl backdrop-blur-md md:p-10">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
        {/* To Review */}
        {activeTab === 'toReview' &&
          toReviewProducts?.data.map((product:any) => (
            <div
              key={product.id}
            ...
            >
              ...
              {product?.product?.review?.length === 0 ? (
                ...
                <div className='font-bold text-lg'>
                  {product.product.review.reduce((acc:number, d:any) => acc + d.rate, 0)}{" "}★
                </div>
              )}
      ...
      {activeTab === 'toFollowUp' && (
        <>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
            {data?.toReview.data.map((product:any) => (
              <div
                key={product.id}
      ...
      {activeTab === 'history' && (
        <>
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
            {data?.history.data.map((product:any) =>
                ...
                >
                  ...
                  {review.attachments.images.length > 0 && (
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      {review.attachments.images.map((img:any, index:number) => (
                        <Image
                          key={index}
                          src={img}
